...
% c = fdcoeffF(k,0,j);      % coefficients for k'th derivative at x=0
c = fdcoeffF(k,0,j);      % coefficients for k'th derivative at x=1

% print out stencil:

disp(' ')
fprintf('The derivative u^(%i) of u at x0 is approximated by\n',k)
disp(' ')
fprintf('          1/h^%i * [\n',k)
for i=1:n-1
  if j(i) < 0
    fprintf('                     %22.15e * u(x0%i*h) + \n',c(i),j(i))
  elseif j(i) == 0
    fprintf('                     %22.15e * u(x0) + \n',c(i))
  else
    fprintf('                     %22.15e * u(x0+%i*h) + \n',c(i),j(i))
...
disp(' ')
disp('For smooth u,')
fprintf('       Error = %g * h^%i*u^(%i) + %g * h^%i*u^(%i) + ...\n',err0,n-k,n,err1,n-k+1,n+1)
