%function [h,k,error] = advdiff_CN(m)
%
% heat_CN.m
%
...
for ii=1:length(mvals)
    ...
    k = h^2; %in this case you need k=O(h^2) to h of order 2
    % but the method is still order 1 in time %%bad
    % time step
    kvals = [kvals k];
...
%     end
    ...
    beta = 150;
    vt = 0.4;
    utrue = @(x,t) exp(-(x-vt).^2 / (4*kappa*t + 1/beta)) / sqrt(4*beta*kappa*t+1);

    % initial conditions:
    u0 = utrue(x,0);
    v = vt/nsteps; %velocity %ony way to get the units straight
    ...
    r2 = (1/2) * kappa* k/(h^2);
    e = ones(m,1);
    A1 = spdiags([-e 0*e e], -1:1,m,m); %for advection
    A2 = spdiags([e -2*e e], -1:1, m, m); %for diffusion
    B1 = eye(m) - (r2*A2) + (r1*A1);
    B2 = eye(m) + (r2*A2)-(r1*A1);
    ...
    for n = 1:nsteps
        ...
        rhs(1) = rhs(1) + r1*(g0n + g0np) + r2*(g0np + g0n);
        rhs(m) = rhs(m) - r1*(g1n + g1np) + r2*(g1n + g1np);
