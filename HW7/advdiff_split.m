%function [h,k,error] = advdiff_split(m)
%
% heat_CN.m
%
...
%     end
    ...
    % initial conditions:
    u0 = utrue(x,0);
    v = vt/nsteps; %velocity %only way to get the units straight

    % Each time step we solve MOL system U' = AU + g using the Trapezoidal method

    % set up matrices:
    r1 = v*k/(h); %for leapfrog
    r2 = (1/2) * kappa* k/(h^2); %for CN
    e = ones(m,1);
    A1 = spdiags([-e 0*e e], -1:1,m,m); %for advection leapfrog
    A2 = spdiags([e -2*e e], -1:1, m, m); %for diffusion CN
    B1 = r1*A1;
