function [h,k,error,xvec,tvec,domega] = advection_lf_pbc_c(m)
%
...
tvec(1) = 0;
tvec(2) = tnp;
domega = a*cos(150*h)/(sqrt(1-nu^2*sin(150*h)^2));
for n = 2:nsteps
     tnp = tn + k;   % = t_{n+1}
     ...
     [~,argmax] = max(u); %index at which max occurs
     xind = x(argmax); %use index to get correspondent x value
     
     xvec = [xvec xind];
     tvec = [tvec tnp];     
     % plot results at desired times:
     if mod(n,25)==0 || n==nsteps
